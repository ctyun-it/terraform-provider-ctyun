...
// RocketmqSpecShrinkApi
/* 规格缩容。
 */type RocketmqSpecShrinkApi struct {
	template core.CtyunRequestTemplate
	client   *core.CtyunClient
}

func NewRocketmqSpecShrinkApi(client *core.CtyunClient) *RocketmqSpecShrinkApi {
	return &RocketmqSpecShrinkApi{
		client: client,
		template: core.CtyunRequestTemplate{
			EndpointName: EndpointName,
			Method:       http.MethodPost,
			UrlPath:      "/v3/instance/specShrink",
			ContentType:  "application/json",
		},
	}
}

func (a *RocketmqSpecShrinkApi) Do(ctx context.Context, credential core.Credential, req *RocketmqSpecShrinkRequest) (*RocketmqSpecShrinkResponse, error) {
	builder := core.NewCtyunRequestBuilder(a.template)
	builder.WithCredential(credential)
	ctReq := builder.Build()
	ctReq.AddHeader("regionId", req.RegionId)
	_, err := ctReq.WriteJson(struct {
		*RocketmqSpecShrinkRequest
	...
	var resp RocketmqSpecShrinkResponse
...
type RocketmqSpecShrinkRequest struct {
...
type RocketmqSpecShrinkResponse struct {
	StatusCode string                               `json:"statusCode"` /*  响应状态码。<br>- 800：成功。<br>- 900：失败。  */
	Message    string                               `json:"message"`    /*  描述状态。  */
	ReturnObj  *RocketmqSpecShrinkReturnObjResponse `json:"returnObj"`  /*  返回对象。  */
	Error      string                               `json:"error"`      /*  错误码，只有失败才显示，参见错误码说明。  */
}

type RocketmqSpecShrinkReturnObjResponse struct {
	Data *RocketmqSpecShrinkReturnObjDataResponse `json:"data"` /*  返回数据。  */
}

type RocketmqSpecShrinkReturnObjDataResponse struct{}
