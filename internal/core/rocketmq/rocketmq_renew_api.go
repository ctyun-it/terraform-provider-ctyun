...
// RocketmqRenewApi
/* 续订实例
 */type RocketmqRenewApi struct {
	template core.CtyunRequestTemplate
	client   *core.CtyunClient
}

func NewRocketmqRenewApi(client *core.CtyunClient) *RocketmqRenewApi {
	return &RocketmqRenewApi{
		client: client,
		template: core.CtyunRequestTemplate{
			EndpointName: EndpointName,
			Method:       http.MethodPost,
			UrlPath:      "/v3/instance/renew",
			ContentType:  "application/json",
		},
	}
}

func (a *RocketmqRenewApi) Do(ctx context.Context, credential core.Credential, req *RocketmqRenewRequest) (*RocketmqRenewResponse, error) {
	builder := core.NewCtyunRequestBuilder(a.template)
	builder.WithCredential(credential)
	ctReq := builder.Build()
	ctReq.AddHeader("regionId", req.RegionId)
	_, err := ctReq.WriteJson(struct {
		*RocketmqRenewRequest
	...
	var resp RocketmqRenewResponse
...
type RocketmqRenewRequest struct {
...
type RocketmqRenewResponse struct {
	StatusCode string                          `json:"statusCode"` /*  响应状态码。<br>- 800：成功。<br>- 900：失败。  */
	Message    string                          `json:"message"`    /*  描述状态。  */
	ReturnObj  *RocketmqRenewReturnObjResponse `json:"returnObj"`  /*  返回对象。  */
	Error      string                          `json:"error"`      /*  错误码，只有失败才显示，参见错误码说明。  */
}

type RocketmqRenewReturnObjResponse struct {
