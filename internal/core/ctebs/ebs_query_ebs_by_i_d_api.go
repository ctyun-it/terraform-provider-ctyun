...
type EbsQueryEbsByIDReturnObjResponse struct {
	...
	DiskID   string `json:"diskID,omitempty"`   /*  云硬盘ID。  */
	DiskSize int64  `json:"diskSize,omitempty"` /*  磁盘大小，单位为GB。  */
	...
	DiskFreeze        *bool                                          `json:"diskFreeze"`                 /*  云硬盘是否已冻结。  */
	ProvisionedIops   int64                                          `json:"provisionedIops,omitempty"`  /*  XSSD类型盘的预配置iops，未配置返回0，其他类型盘不返回。  */
	...
	DeleteSnapWithEbs string                                         `json:"deleteSnapWithEbs"`
}
