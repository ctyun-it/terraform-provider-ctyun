...
type EbsNewEbsRequest struct {
	...
	MultiAttach *bool  `json:"multiAttach"`           /*  是否多云主机挂载，取值范围：●true：是●false：否默认值：false  */
	IsEncrypt   *bool  `json:"isEncrypt"`             /*  是否加密盘，取值范围：●true：是●false：否默认值：false 共享盘、ISCSI模式磁盘、极速型SSD类型盘、XSSD系列盘不支持加密。  */
	...
	DiskSize int64 `json:"diskSize,omitempty"` /*  云硬盘大小，单位GB。数据盘的取值范围：
	●超高IO/高IO/极速型SSD/普通IO：10GB~32768GB
	●XSSD-0：10GB-65536GB
	●XSSD-1：20GB-65536GB
	●XSSD-2：512GB-65536GB  */
	OnDemand   *bool  `json:"onDemand"`            /*  是否按需下单，取值范围：●true：是●false：否 默认值：true  */
	CycleType  string `json:"cycleType,omitempty"` /*  包周期类型，year：年，month：月。onDemand为false时，必须指定year和month的值。  */
	CycleCount int32  `json:"cycleCount"`          /*  包周期数。onDemand为false时必须指定。周期为年（year）时，周期最大长度不能超过5年。周期为月（month）时，周期最大长度不能超过60月。  */
	...
	ProvisionedIops int64  `json:"provisionedIops,omitempty"` /*  XSSD类型盘的预配置iops值，最小值为1，其他类型的盘不支持设置。具体取值范围如下：
