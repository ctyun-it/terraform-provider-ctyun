...
// CdaCdaPhysicalLineListApi
/* 查询已创建的物理专线 */
type CdaCdaPhysicalLineListApi struct {
	template core.CtyunRequestTemplate
	client   *core.CtyunClient
}

func NewCdaCdaPhysicalLineListApi(client *core.CtyunClient) *CdaCdaPhysicalLineListApi {
	return &CdaCdaPhysicalLineListApi{
		client: client,
		template: core.CtyunRequestTemplate{
			EndpointName: EndpointName,
			Method:       http.MethodGet,
			UrlPath:      "/v4/cda/physical-line/list",
			ContentType:  "application/json",
		},
	}
}

func (a *CdaCdaPhysicalLineListApi) Do(ctx context.Context, credential core.Credential, req *CdaCdaPhysicalLineListRequest) (*CdaCdaPhysicalLineListResponse, error) {
	builder := core.NewCtyunRequestBuilder(a.template)
	builder.WithCredential(credential)
	ctReq := builder.Build()
	_, err := ctReq.WriteJson(struct {
		*CdaCdaPhysicalLineListRequest
	}{
		req,
	...
	var resp CdaCdaPhysicalLineListResponse
...
type CdaCdaPhysicalLineListRequest struct {
...
type CdaCdaPhysicalLineListResponse struct {
	StatusCode        *int32  `json:"statusCode"`        /*  返回状态码(800为成功，900为失败)  */
	Message           *string `json:"message"`           /*  返回信息  */
	SubCode           *string `json:"subCode"`           /*  返回码  */
	SubMsg            *string `json:"subMsg"`            /*  返回码相应描述  */
	TotalCount        *int32  `json:"totalCount"`        /*  数量总数  */
