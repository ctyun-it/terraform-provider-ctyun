...
// CdaCdaBgpRouteAddApi
/* 创建BGP动态路由 */
type CdaCdaBgpRouteAddApi struct {
	template core.CtyunRequestTemplate
	client   *core.CtyunClient
}

func NewCdaCdaBgpRouteAddApi(client *core.CtyunClient) *CdaCdaBgpRouteAddApi {
	return &CdaCdaBgpRouteAddApi{
		client: client,
		template: core.CtyunRequestTemplate{
			EndpointName: EndpointName,
			Method:       http.MethodPost,
			UrlPath:      "/v4/cda/bgp-route/add",
			ContentType:  "application/json",
		},
	}
}

func (a *CdaCdaBgpRouteAddApi) Do(ctx context.Context, credential core.Credential, req *CdaCdaBgpRouteAddRequest) (*CdaCdaBgpRouteAddResponse, error) {
	builder := core.NewCtyunRequestBuilder(a.template)
	builder.WithCredential(credential)
	ctReq := builder.Build()
	_, err := ctReq.WriteJson(struct {
		*CdaCdaBgpRouteAddRequest
	}{
		req,
	...
	var resp CdaCdaBgpRouteAddResponse
...
type CdaCdaBgpRouteAddRequest struct {
	...
	BGPList          []*CdaCdaBgpRouteAddBGPListRequest     `json:"BGPList,omitempty"`          /*  IPv4类型的BGP列表  */
	BGPIpv6List      []*CdaCdaBgpRouteAddBGPIpv6ListRequest `json:"BGPIpv6List,omitempty"`      /*  IPv6类型的BGP列表  */
...
type CdaCdaBgpRouteAddBGPListRequest struct {
...
type CdaCdaBgpRouteAddBGPIpv6ListRequest struct {
	BGPNeighbor   string    `json:"BGPNeighbor"`             /*  BGP邻居名称  */
	BGPIP         string    `json:"BGPIP"`                   /*  BGP邻居IP(物理专线的远端互联IP)  */
	LineID        string    `json:"lineID"`                  /*  物理专线ID  */
	PeerAS        string    `json:"peerAS"`                  /*  Peer AS号  */
...
type CdaCdaBgpRouteAddResponse struct {
	StatusCode  *int32                                `json:"statusCode"`  /*  返回状态码(800为成功，900为失败)  */
	Message     *string                               `json:"message"`     /*  失败时的错误描述，一般为英文描述  */
	Description *string                               `json:"description"` /*  失败时的错误描述，一般为中文描述  */
	ReturnObj   *CdaCdaBgpRouteAddReturnObjResponse   `json:"returnObj"`   /*  返回对象  */
	ErrorCode   *string                               `json:"errorCode"`   /*  业务细分码，为product.module.code三段式码  */
	ErrorDetail *CdaCdaBgpRouteAddErrorDetailResponse `json:"errorDetail"` /*  错误明细  */
	Error       *string                               `json:"error"`       /*  业务细分码，为product.module.code三段式码  */
}

type CdaCdaBgpRouteAddReturnObjResponse struct {
...
type CdaCdaBgpRouteAddErrorDetailResponse struct{}
