...
// CfCreateWorkflowApi
/* 创建云工作流 */
type CfCreateWorkflowApi struct {
	template core.CtyunRequestTemplate
	client   *core.CtyunClient
}

func NewCfCreateWorkflowApi(client *core.CtyunClient) *CfCreateWorkflowApi {
	return &CfCreateWorkflowApi{
		client: client,
		template: core.CtyunRequestTemplate{
...
func (a *CfCreateWorkflowApi) Do(ctx context.Context, credential core.Credential, req *CfCreateWorkflowRequest) (*CfCreateWorkflowResponse, error) {
	builder := core.NewCtyunRequestBuilder(a.template)
	builder.WithCredential(credential)
	ctReq := builder.Build()
	ctReq.AddHeader("regionId", req.RegionId)
	_, err := ctReq.WriteJson(struct {
		*CfCreateWorkflowRequest
	...
	var resp CfCreateWorkflowResponse
...
type CfCreateWorkflowRequest struct {
	...
	Name             string  `json:"name"`                  /*  工作流名称，只能包含字母、数字和横杠，最大长度128  */
...
type CfCreateWorkflowResponse struct {
	StatusCode *int32                             `json:"statusCode"` /*  状态码，0表示成功，非0表示不成功  */
	Error      *string                            `json:"error"`      /*  错误码  */
	Message    *string                            `json:"message"`    /*  结果描述  */
	ReturnObj  *CfCreateWorkflowReturnObjResponse `json:"returnObj"`  /*  结果数据  */
}

type CfCreateWorkflowReturnObjResponse struct {
