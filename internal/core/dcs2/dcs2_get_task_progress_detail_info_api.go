...
type Dcs2GetTaskProgressDetailInfoReturnObjDataSyncCountInfoResponse struct {
	ReadCount  int64   `json:"readCount"`  /*  从源端读取的总命令数。  */
	ReadOps    float64 `json:"readOps"`    /*  从源端读取的命令总OPS。  */
	WriteCount int64   `json:"writeCount"` /*  发送给目标的总命令数。  */
...
type Dcs2GetTaskProgressDetailInfoReturnObjSourceProgressInfoListResponse struct {
	Address       string  `json:"address"`       /*  源端ip:port。  */
	SyncPercent   float64 `json:"syncPercent"`   /*  全量数据的同步百分比（不包含增量）。  */
	AofOffsetDiff int64   `json:"aofOffsetDiff"` /*  AOF偏移差距（增量同步时有）。  */
	State         string  `json:"state"`         /*  同步阶段。<li>unknown status<li>hand shaking<li>waiting bgsave<li>receiving rdb<li>syncing rdb<li>syncing aof（增量同步时有）  */
}
