...
type CtecsUpdateEcsLabelV41Request struct {
	...
	Action     string                                    `json:"action"`              /*  操作类型，可选值：ADD（增加）、UPDATE（修改）、DELETE（删除）  */
	LabelList  []*CtecsUpdateEcsLabelV41LabelListRequest `json:"labelList,omitempty"` /*  标签列表  */
}

type CtecsUpdateEcsLabelV41LabelListRequest struct {
	LabelKey   string `json:"labelKey"`   /*  标签键  */
	LabelValue string `json:"labelValue"` /*  标签值  */
}
