...
// AmqpCreateOrderApi
/* 创建按需计费实例。
 */type AmqpCreateOrderApi struct {
	template core.CtyunRequestTemplate
	client   *core.CtyunClient
}

func NewAmqpCreateOrderApi(client *core.CtyunClient) *AmqpCreateOrderApi {
	return &AmqpCreateOrderApi{
		client: client,
		template: core.CtyunRequestTemplate{
...
func (a *AmqpCreateOrderApi) Do(ctx context.Context, credential core.Credential, req *AmqpCreateOrderRequest) (*AmqpCreateOrderResponse, error) {
	...
	var resp AmqpCreateOrderResponse
...
type AmqpCreateOrderRequest struct {
...
type AmqpCreateOrderResponse struct {
	StatusCode string                            `json:"statusCode"` /*  响应状态码。<br>- 800：成功。<br>- 900：失败。  */
	Message    string                            `json:"message"`    /*  描述状态。  */
	ReturnObj  *AmqpCreateOrderReturnObjResponse `json:"returnObj"`  /*  返回对象。  */
	Error      string                            `json:"error"`      /*  错误码，只有失败才显示，参见错误码说明。  */
}

type AmqpCreateOrderReturnObjResponse struct {
	Data *AmqpCreateOrderReturnObjDataResponse `json:"data"` /*  返回数据。  */
}

type AmqpCreateOrderReturnObjDataResponse struct{}
