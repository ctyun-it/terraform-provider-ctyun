...
// AmqpCreatePostPayOrderApi
/* 创建按需计费实例。
 */type AmqpCreatePostPayOrderApi struct {
	template core.CtyunRequestTemplate
	client   *core.CtyunClient
}

func NewAmqpCreatePostPayOrderApi(client *core.CtyunClient) *AmqpCreatePostPayOrderApi {
	return &AmqpCreatePostPayOrderApi{
		client: client,
		template: core.CtyunRequestTemplate{
...
func (a *AmqpCreatePostPayOrderApi) Do(ctx context.Context, credential core.Credential, req *AmqpCreatePostPayOrderRequest) (*AmqpCreatePostPayOrderResponse, error) {
	...
	var resp AmqpCreatePostPayOrderResponse
...
type AmqpCreatePostPayOrderRequest struct {
...
type AmqpCreatePostPayOrderResponse struct {
	StatusCode string                                   `json:"statusCode"` /*  响应状态码。<br>- 800：成功。<br>- 900：失败。  */
	Message    string                                   `json:"message"`    /*  描述状态。  */
	ReturnObj  *AmqpCreatePostPayOrderReturnObjResponse `json:"returnObj"`  /*  返回对象。  */
	Error      string                                   `json:"error"`      /*  错误码，只有失败才显示，参见错误码说明。  */
}

type AmqpCreatePostPayOrderReturnObjResponse struct {
	Data *AmqpCreatePostPayOrderReturnObjDataResponse `json:"data"` /*  返回数据。  */
}

type AmqpCreatePostPayOrderReturnObjDataResponse struct{}
