...
func TestRequestRecoverRetry5xx(t *testing.T) {
	...
	out := &testData{}
	r := s.NewRequest(&Operation{Name: "Operation"}, nil, out)
	err := r.Send()
	if err != nil {
...
func TestRequestExhaustRetries(t *testing.T) {
	...
	r := s.NewRequest(&Operation{Name: "Operation"}, nil, nil)
	err := r.Send()
	if err == nil {
		t.Fatalf("expect error, but did not get one")
	}
...
func TestMakeAddtoUserAgentHandler(t *testing.T) {
	fn := MakeAddToUserAgentHandler("name", "version", "extra1", "extra2")
	r := &Request{HTTPRequest: &http.Request{Header: http.Header{}}}
...
func TestMakeAddtoUserAgentFreeFormHandler(t *testing.T) {
	fn := MakeAddToUserAgentFreeFormHandler("name/version (extra1; extra2)")
	r := &Request{HTTPRequest: &http.Request{Header: http.Header{}}}
	r.HTTPRequest.Header.Set("User-Agent", "foo/bar")
	fn(r)
...
func TestRequestUserAgent(t *testing.T) {
	...
	req := s.NewRequest(&Operation{Name: "Operation"}, nil, &testData{})
...
func TestIsSerializationErrorRetryable(t *testing.T) {
	testCases := []struct {
		...
		{
			err:      awserr.New(ErrCodeSerialization, "foo error", nil),
			expected: false,
		},
		{
		...
		{
			err:      awserr.New(ErrCodeSerialization, "foo error", errAcceptConnectionResetStub),
			expected: true,
		},
	}
...
func TestWithLogLevel(t *testing.T) {
	...
	opt := WithLogLevel(aws.LogDebugWithHTTPBody)
	r.ApplyOptions(opt)
...
func TestWithGetResponseHeader(t *testing.T) {
	...
	var val, val2 string
	r.ApplyOptions(
		WithGetResponseHeader("x-a-header", &val),
		WithGetResponseHeader("x-second-header", &val2),
	)
...
func TestWithGetResponseHeaders(t *testing.T) {
	...
	opt := WithGetResponseHeaders(&headers)
...
func (d *testRetryer) RetryRules(r *Request) time.Duration {
	return 0
}

func (d *testRetryer) ShouldRetry(r *Request) bool {
...
func TestEnforceShouldRetryCheck(t *testing.T) {
	...
	s.Handlers.Validate.Clear()
	s.Handlers.Send.Swap(corehandlers.SendHandler.Name, NamedHandler{
...
func TestIsNoBodyReader(t *testing.T) {
	...
	for i, c := range cases {
		if e, a := c.expect, NoBody == c.reader; e != a {
...
func TestRequest_TemporaryRetry(t *testing.T) {
	...
	req := svc.NewRequest(&Operation{
		Name: "name", HTTPMethod: "GET", HTTPPath: "/path",
	}, &struct{}{}, &struct{}{})

	req.Handlers.Unmarshal.PushBack(func(r *Request) {
		defer req.HTTPResponse.Body.Close()
		_, err := io.Copy(ioutil.Discard, req.HTTPResponse.Body)
		r.Error = awserr.New(ErrCodeSerialization, "error", err)
	})
	...
	if e, a := ErrCodeSerialization, aerr.Code(); e != a {
		t.Errorf("expect %q error code, got %q", e, a)
	}
...
func TestRequest_Presign(t *testing.T) {
	presign := func(r *Request, expire time.Duration) (string, http.Header, error) {
		u, err := r.Presign(expire)
		return u, nil, err
	}
	presignRequest := func(r *Request, expire time.Duration) (string, http.Header, error) {
	...
	cases := []struct {
		...
		PresignFn func(*Request, time.Duration) (string, http.Header, error)
		SignerFn  func(*Request)
		URL       string
		Header    http.Header
		Err       string
	}{
		{
			PresignFn: presign,
			Err:       ErrCodeInvalidPresignExpire,
		},
		{
		...
		},
		{
			...
			PresignFn: func(r *Request, dur time.Duration) (string, http.Header, error) {
				r.NotHoist = true
				return presign(r, dur)
			},
			SignerFn: func(r *Request) {
...
func TestRequestWillRetry_ByBody(t *testing.T) {
	...
	for i, c := range cases {
		...
		if e, a := c.IsReqNoBody, NoBody == req.HTTPRequest.Body; e != a {
...
func TestRequestEndpointWithDefaultPort(t *testing.T) {
	...
	r.Handlers.Send.PushFront(func(r *Request) {
		req := r.HTTPRequest
...
func TestRequestMarshaledEndpointWithDefaultPort(t *testing.T) {
	...
	r.Handlers.Validate.Clear()
	r.Handlers.ValidateResponse.Clear()
	r.Handlers.Build.PushBack(func(r *Request) {
		req := r.HTTPRequest
