...
func (c *mockClient) MockRequest(input *MockInput) (*Request, *MockOutput) {
	op := &Operation{
		Name:       "Mock",
		HTTPMethod: "POST",
		HTTPPath:   "/",
...
func BuildNewMockRequest(c *mockClient, in *MockInput) func([]Option) (*Request, error) {
	return func(opts []Option) (*Request, error) {
...
func TestWaiterPathAll(t *testing.T) {
	...
	numBuiltReq := 0
	svc.Handlers.Build.PushBack(func(r *Request) {
		numBuiltReq++
	})
	svc.Handlers.Unmarshal.PushBack(func(r *Request) {
		if reqNum >= len(resps) {
	...
	w := Waiter{
		MaxAttempts:      10,
		Delay:            ConstantWaiterDelay(0),
		SleepWithContext: aws.SleepWithContext,
		Acceptors: []WaiterAcceptor{
			{
				State:    SuccessWaiterState,
				Matcher:  PathAllWaiterMatch,
...
func TestWaiterFailure(t *testing.T) {
	...
	w := Waiter{
		...
		Acceptors: []WaiterAcceptor{
			...
			{
				State:    FailureWaiterState,
				Matcher:  PathAnyWaiterMatch,
				Argument: "States[].State",
				Expected: "stopping",
			},
		},
	...
	if e, a := WaiterResourceNotReadyErrorCode, err.Code(); e != a {
		t.Errorf("expect %v, got %v", e, a)
	}
...
func TestWaiterError(t *testing.T) {
	...
	svc.Handlers.Send.PushBack(func(r *Request) {
		code := 200
		if reqNum == 1 {
			code = 400
		}
	...
	svc.Handlers.UnmarshalMeta.PushBack(func(r *Request) {
	...
	w := Waiter{
		...
		Acceptors: []WaiterAcceptor{
			...
			{
				State:    RetryWaiterState,
				Matcher:  ErrorWaiterMatch,
				Argument: "",
	...
	if e, a := WaiterResourceNotReadyErrorCode, aerr.Code(); e != a {
...
func TestWaiter_ApplyOptions(t *testing.T) {
	...
	w.ApplyOptions(
		WithWaiterLogger(logger),
		WithWaiterRequestOptions(WithLogLevel(aws.LogDebug)),
		WithWaiterMaxAttempts(2),
		WithWaiterDelay(ConstantWaiterDelay(5*time.Second)),
	)
...
func TestWaiter_WithContextCanceled(t *testing.T) {
	...
	w := Waiter{
		Name:             "TestWaiter",
		MaxAttempts:      10,
		Delay:            ConstantWaiterDelay(1 * time.Millisecond),
		SleepWithContext: aws.SleepWithContext,
		Acceptors: []WaiterAcceptor{
			{
				State:    SuccessWaiterState,
		...
		Logger: aws.NewDefaultLogger(),
		NewRequest: func(opts []Option) (*Request, error) {
			req := c.NewRequest(&Operation{Name: "Operation"}, nil, nil)
	...
	aerr := err.(awserr.Error)
	if e, a := CanceledErrorCode, aerr.Code(); e != a {
		t.Errorf("expect %q error code, got %q", e, a)
	}
...
func TestWaiterNilInput(t *testing.T) {
	...
	client.Handlers.Send.PushBack(func(r *Request) {
		r.HTTPResponse = &http.Response{
...
func TestWaiterWithContextNilInput(t *testing.T) {
	...
	err := client.WaitUntilBucketExistsWithContext(ctx, nil,
		WithWaiterDelay(ConstantWaiterDelay(0)),
		WithWaiterMaxAttempts(1),
	)
	if err != nil {
		t.Fatalf("expect no error, but got %v", err)
	}
