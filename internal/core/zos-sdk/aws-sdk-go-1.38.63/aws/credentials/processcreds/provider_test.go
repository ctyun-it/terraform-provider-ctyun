...
func TestProcessProviderBadCommand(t *testing.T) {
	...
	creds := NewCredentials("/bad/process")
	_, err := creds.Get()
	if err.(awserr.Error).Code() != ErrCodeProcessProviderExecution {
		t.Errorf("expected %v, got %v", ErrCodeProcessProviderExecution, err)
	}
}
...
func TestProcessProviderExpectErrors(t *testing.T) {
	...
	_, err := creds.Get()
	if err.(awserr.Error).Code() != ErrCodeProcessProviderParse {
		t.Errorf("expected %v, got %v", ErrCodeProcessProviderParse, err)
	}
	...
	creds = NewCredentials(
		fmt.Sprintf("%s %s",
			...
			strings.Join(
				...
				string(os.PathSeparator))))
	_, err = creds.Get()
	if err.(awserr.Error).Code() != ErrCodeProcessProviderVersion {
		t.Errorf("expected %v, got %v", ErrCodeProcessProviderVersion, err)
	}
	...
	creds = NewCredentials(
		fmt.Sprintf(
			...
			strings.Join(
				...
				string(os.PathSeparator))))
	_, err = creds.Get()
	if err.(awserr.Error).Code() != ErrCodeProcessProviderRequired {
		t.Errorf("expected %v, got %v", ErrCodeProcessProviderRequired, err)
	}
...
func TestProcessProviderTimeout(t *testing.T) {
	...
	creds := NewCredentialsTimeout(
		command,
		time.Duration(1)*time.Second)
	if _, err := creds.Get(); err == nil || err.(awserr.Error).Code() != ErrCodeProcessProviderExecution || err.(awserr.Error).Message() != "credential process timed out" {
		t.Errorf("expected %v, got %v", ErrCodeProcessProviderExecution, err)
	}
	...
	myCommand := exec.Command(
		fmt.Sprintf(
			...
			strings.Join(
				[]string{"testdata", "static.json"},
				string(os.PathSeparator))))
	creds := NewCredentialsCommand(myCommand, func(opt *ProcessProvider) {
