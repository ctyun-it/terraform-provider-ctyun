...
func ExampleEnumPartitions() {
	...
	partitions := resolver.(EnumPartitions).Partitions()
...
func ExampleResolverFunc() {
	myCustomResolver := func(service, region string, optFns ...func(*Options)) (ResolvedEndpoint, error) {
		if service == S3ServiceID {
	...
	sess := session.Must(session.NewSession(&aws.Config{
		...
		EndpointResolver: ResolverFunc(myCustomResolver),
	}))
