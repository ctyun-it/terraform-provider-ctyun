...
func TestPresignHandler(t *testing.T) {
	...
	svc.Handlers.Sign.SwapNamed(request.NamedHandler{
		Name: SignRequestHandler.Name,
		Fn: func(r *request.Request) {
			SignSDKRequestWithCurrentTime(r, epochTime)
		},
	})
...
func TestStandaloneSign_CustomURIEscape(t *testing.T) {
	...
	signer := NewSigner(creds, func(s *Signer) {
...
func TestStandaloneSign_WithPort(t *testing.T) {
	...
	for _, c := range cases {
		signer := NewSigner(unit.Session.Config.Credentials)
