 ...
 */type CtimageListImagesApi struct {
	template core.CtyunRequestTemplate
	client   *core.CtyunClient
}

func NewCtimageListImagesApi(client *core.CtyunClient) *CtimageListImagesApi {
	return &CtimageListImagesApi{
		client: client,
		template: core.CtyunRequestTemplate{
...
func (a *CtimageListImagesApi) Do(ctx context.Context, credential core.Credential, req *CtimageListImagesRequest) (*CtimageListImagesResponse, error) {
	...
	if req.CwaiType != "" {
		ctReq.AddParam("cwaiType", req.CwaiType)
	}
	if req.FlavorName != "" {
		ctReq.AddParam("flavorName", req.FlavorName)
	}
	if req.ImageScene != "" {
		ctReq.AddParam("imageScene", req.ImageScene)
	}
	if req.ImageStatus != "" {
		ctReq.AddParam("imageStatus", req.ImageStatus)
	}
	if req.ImageSubcategory != "" {
		ctReq.AddParam("imageSubcategory", req.ImageSubcategory)
	}
	if req.ImageVisibilityCode != nil {
		ctReq.AddParam("imageVisibilityCode", strconv.FormatInt(int64(*req.ImageVisibilityCode), 10))
	}
	if req.PageNo != nil {
	...
	var resp CtimageListImagesResponse
...
type CtimageListImagesRequest struct {
	...
	AzName              string `json:"azName,omitempty"`              /*  可用区名称。注意：<br />1. 仅在多可用区资源池下对物理机镜像生效。请不要在其它场景使用此参数<br />2. 可使用<a href="https://eop.ctyun.cn/ebp/ctapiDocument/search?sid=25&api=5855&data=87&isNormal=1&vid=81" target="_blank">资源池可用区查询</a>接口查询多可用区资源池的可用区信息。若响应的 returnObj 中的 zoneList 是空的可用区列表，则所指定的资源池是单可用区资源池  */
	CwaiType            string `json:"cwaiType,omitempty"`            /*  云骁智算云主机节点类型。取值范围（值：描述）：<br />control：控制面云主机节点，<br />node：GPU 云主机节点<br /><br />注意：仅在 imageVisibilityCode 参数值为 1（公共镜像）时用于限制仅显示适用于指定云骁智算云主机节点类型的公共镜像，未指定节点类型时不启用此限制。请不要在其它场景使用此参数  */
	FlavorName          string `json:"flavorName,omitempty"`          /*  规格名称。注意：<br />1. 可使用<a href="https://eop.ctyun.cn/ebp/ctapiDocument/search?sid=25&api=8327&data=87&isNormal=1&vid=81" target="_blank">查询一个或多个云主机规格资源</a>接口来查询您可以使用的云主机规格（如 s7.small.1）<br />2. 可使用<a href="https://eop.ctyun.cn/ebp/ctapiDocument/search?sid=25&api=11998&data=87&isNormal=1&vid=81" target="_blank">查询轻量型云主机的规格套餐资源</a>接口来查询您可以使用的轻量型云主机规格（如 lite1.fix.small.1）  */
	ImageScene          string `json:"imageScene,omitempty"`          /*  镜像场景。取值范围（值：描述）：<br />dev：开发工具，<br />ecommerce：电商，<br />gaming：游戏，<br />website：网站<br /><br />注意：仅在 imageVisibilityCode 参数值为 5（应用镜像）时用于筛选应用镜像，未指定镜像场景时不启用此筛选。请不要在其它场景使用此参数  */
	ImageStatus         string `json:"imageStatus,omitempty"`         /*  镜像状态。取值范围（值：描述）：<br />accepted：已接受共享镜像，<br />rejected：已拒绝共享镜像，<br />waiting：等待接受/拒绝共享镜像<br /><br />注意：仅在 imageVisibilityCode 参数值为 2（共享镜像）时生效，未指定镜像状态时默认显示镜像状态为 accepted 或 waiting 的共享镜像。请不要在其它场景使用此参数  */
	ImageSubcategory    string `json:"imageSubcategory,omitempty"`    /*  镜像子种类。取值范围（值：描述）：<br />app：云主机应用镜像，<br />thin_app：轻量型云主机应用镜像<br /><br />注意：仅在 imageVisibilityCode 参数值为 5（应用镜像）时用于筛选应用镜像，未指定镜像子种类时不启用此筛选。请不要在其它场景使用此参数  */
	ImageVisibilityCode *int32 `json:"imageVisibilityCode,omitempty"` /*  镜像可见类型代码，详见枚举值表格  */
	PageNo              *int32 `json:"pageNo,omitempty"`              /*  页码。取值范围：最小 1（默认值）  */
	PageSize            *int32 `json:"pageSize,omitempty"`            /*  每页记录数目。取值范围：最小 1，最大 50，默认 10  */
	ProjectID           string `json:"projectID,omitempty"`           /*  企业项目 ID。可使用<a href="https://eop.ctyun.cn/ebp/ctapiDocument/search?sid=77&api=7246&data=114&isNormal=1&vid=107" target="_blank">查询企业项目列表</a>接口来查询您可以使用的企业项目 ID。注意：仅在 imageVisibilityCode 参数值为 0（私有镜像）时生效。请不要在其它场景使用此参数  */
	QueryContent        string `json:"queryContent,omitempty"`        /*  查询内容  */
}

type CtimageListImagesResponse struct {
	...
	ReturnObj   *CtimageListImagesReturnObjResponse `json:"returnObj"`             /*  成功时返回的数据  */
}

type CtimageListImagesReturnObjResponse struct {
	Images       []*CtimageListImagesReturnObjImagesResponse `json:"images"`                 /*  镜像列表  */
...
type CtimageListImagesReturnObjImagesResponse struct {
