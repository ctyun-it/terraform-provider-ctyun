...
// CtimageCreateEcsSnapshotSystemDiskImageApi
/* 使用指定的云主机快照来创建 1 份私有系统盘镜像。<br />准备：<br />1. 在调用前需了解如何构造请求，可参见：如何调用 API - 构造请求。<br />2. OpenAPI 请求需进行加密调用，可参见：如何调用 API - 认证鉴权。<br />注意：在调用前，请您认真阅读此文档，包括但不限于参数描述中的“注意”部分。
 */type CtimageCreateEcsSnapshotSystemDiskImageApi struct {
	template core.CtyunRequestTemplate
	client   *core.CtyunClient
}

func NewCtimageCreateEcsSnapshotSystemDiskImageApi(client *core.CtyunClient) *CtimageCreateEcsSnapshotSystemDiskImageApi {
	return &CtimageCreateEcsSnapshotSystemDiskImageApi{
		client: client,
		template: core.CtyunRequestTemplate{
			EndpointName: EndpointName,
			Method:       http.MethodPost,
			UrlPath:      "/v4/image/create-from-ecs-snapshot",
			ContentType:  "application/json",
		},
	}
}

func (a *CtimageCreateEcsSnapshotSystemDiskImageApi) Do(ctx context.Context, credential core.Credential, req *CtimageCreateEcsSnapshotSystemDiskImageRequest) (*CtimageCreateEcsSnapshotSystemDiskImageResponse, error) {
	builder := core.NewCtyunRequestBuilder(a.template)
	builder.WithCredential(credential)
	ctReq := builder.Build()
	_, err := ctReq.WriteJson(struct {
		*CtimageCreateEcsSnapshotSystemDiskImageRequest
	}{
		req,
	...
	var resp CtimageCreateEcsSnapshotSystemDiskImageResponse
...
type CtimageCreateEcsSnapshotSystemDiskImageRequest struct {
	...
	RegionID                  string                                                  `json:"regionID,omitempty"`        /*  资源池 ID。可使用<a href="https://eop.ctyun.cn/ebp/ctapiDocument/search?sid=25&api=5851&data=87&isNormal=1&vid=81" target="_blank">资源池列表查询</a>接口来查询您可见的资源池的列表。注意：此接口仅支持具备从云主机快照创建私有镜像的功能的多可用区资源池。  */
	SnapshotID                string                                                  `json:"snapshotID,omitempty"`      /*  云主机快照 ID。可使用<a href="https://eop.ctyun.cn/ebp/ctapiDocument/search?sid=25&api=8349&data=87&isNormal=1&vid=81" target="_blank">查询云主机快照列表</a>接口来查询您的云主机快照列表，可使用<a href="https://eop.ctyun.cn/ebp/ctapiDocument/search?sid=25&api=8346&data=87&isNormal=1&vid=81" target="_blank">查询云主机快照详情</a>接口来查询 1 个云主机快照的详细信息。注意：<br />1. 所指定的云主机快照应是云主机快照状态为 available 的云主机快照。<br />2. 所指定的云主机快照包含的系统盘快照应是云硬盘快照状态为 available 且未加密的云硬盘快照。  */
	...
	Labels                    []*CtimageCreateEcsSnapshotSystemDiskImageLabelsRequest `json:"labels"`                    /*  标签列表。注意：<br />1. 列表中最多 10 个标签。<br />2. 标签键不可重复。<br />3. 单个标签键或值应满足长度为 1~32 个字符，不能换行，且不能以空格开头或结尾。  */
	MaximumRAM                int32                                                   `json:"maximumRAM,omitempty"`      /*  最大内存。单位为 GiB。取值范围：0（默认值，即不限制）/1/2/4/8/16/32/64/128/256/512。注意：<br />1. 若同时限制最大和最小内存，则最大内存不能小于最小内存。<br />2. 仅在传入的 regionID 参数所指定的资源池具备设置最大/最小内存的功能时生效。  */
	MinimumRAM                int32                                                   `json:"minimumRAM,omitempty"`      /*  最小内存。单位为 GiB。取值范围：0（默认值，即不限制）/1/2/4/8/16/32/64/128/256/512。注意：<br />1. 若同时限制最大和最小内存，则最大内存不能小于最小内存。<br />2. 仅在传入的 regionID 参数所指定的资源池具备设置最大/最小内存的功能时生效。  */
...
type CtimageCreateEcsSnapshotSystemDiskImageLabelsRequest struct {
	LabelKey   string `json:"labelKey,omitempty"`   /*  标签键。  */
	LabelValue string `json:"labelValue,omitempty"` /*  标签值。  */
}

type CtimageCreateEcsSnapshotSystemDiskImageResponse struct {
	...
	ReturnObj   *CtimageCreateEcsSnapshotSystemDiskImageReturnObjResponse `json:"returnObj"`             /*  成功时返回的数据。  */
}

type CtimageCreateEcsSnapshotSystemDiskImageReturnObjResponse struct {
	Images []*CtimageCreateEcsSnapshotSystemDiskImageReturnObjImagesResponse `json:"images"` /*  镜像列表。应包含 1 个镜像。  */
}

type CtimageCreateEcsSnapshotSystemDiskImageReturnObjImagesResponse struct {
