...
// CtimageCreateEcsSystemDiskImageApi
/* 使用指定的云主机的系统盘（若是在创建时使用 ISO 镜像的云主机，则指其仅有的数据盘）来创建 1 份私有系统盘镜像<br />准备：<br />1. 在调用前需了解如何构造请求，可参见：如何调用 API - 构造请求<br />2. OpenAPI 请求需进行加密调用，可参见：如何调用 API - 认证鉴权<br />注意：在调用前，请您认真阅读此文档，包括但不限于参数描述中的“注意”部分
 */type CtimageCreateEcsSystemDiskImageApi struct {
	template core.CtyunRequestTemplate
	client   *core.CtyunClient
}

func NewCtimageCreateEcsSystemDiskImageApi(client *core.CtyunClient) *CtimageCreateEcsSystemDiskImageApi {
	return &CtimageCreateEcsSystemDiskImageApi{
		client: client,
		template: core.CtyunRequestTemplate{
			EndpointName: EndpointName,
			Method:       http.MethodPost,
			UrlPath:      "/v4/image/create",
			ContentType:  "application/json",
		},
	}
}

func (a *CtimageCreateEcsSystemDiskImageApi) Do(ctx context.Context, credential core.Credential, req *CtimageCreateEcsSystemDiskImageRequest) (*CtimageCreateEcsSystemDiskImageResponse, error) {
	builder := core.NewCtyunRequestBuilder(a.template)
	builder.WithCredential(credential)
	ctReq := builder.Build()
	_, err := ctReq.WriteJson(struct {
		*CtimageCreateEcsSystemDiskImageRequest
	}{
		req,
	...
	var resp CtimageCreateEcsSystemDiskImageResponse
...
type CtimageCreateEcsSystemDiskImageRequest struct {
	...
	Labels                    []*CtimageCreateEcsSystemDiskImageLabelsRequest `json:"labels"`                    /*  标签列表。注意：<br />1. 列表中最多 10 个标签<br />2. 标签键不可重复<br />3. 单个标签键或值应满足长度为 1~32 个字符，不能换行，且不能以空格开头或结尾  */
...
type CtimageCreateEcsSystemDiskImageLabelsRequest struct {
	LabelKey   string `json:"labelKey,omitempty"`   /*  标签键  */
	LabelValue string `json:"labelValue,omitempty"` /*  标签值  */
}

type CtimageCreateEcsSystemDiskImageResponse struct {
	...
	ReturnObj   *CtimageCreateEcsSystemDiskImageReturnObjResponse `json:"returnObj"`             /*  成功时返回的数据  */
}

type CtimageCreateEcsSystemDiskImageReturnObjResponse struct {
	Images []*CtimageCreateEcsSystemDiskImageReturnObjImagesResponse `json:"images"` /*  镜像列表。应包含 1 个镜像  */
}

type CtimageCreateEcsSystemDiskImageReturnObjImagesResponse struct {
