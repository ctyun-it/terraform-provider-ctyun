...
func (c *ctyunEbsBackupRepos) Schema(_ context.Context, _ datasource.SchemaRequest, response *datasource.SchemaResponse) {
	response.Schema = schema.Schema{
		MarkdownDescription: utils.FormatDesc("云硬盘备份存储库", "云硬盘（CT-EVS，Elastic Volume Service）", "https://www.ctyun.cn/document/10026752/10212971`"),
...
func (c *ctyunEbsBackupRepos) Read(ctx context.Context, request datasource.ReadRequest, response *datasource.ReadResponse) {
	...
	for _, repo := range resp.ReturnObj.RepositoryList {
		...
		// 将时间戳转换为UTC字符串
		createTimeStr := time.Unix(int64(repo.CreatedTime), 0).UTC().Format(time.RFC3339)
		expireTimeStr := time.Unix(int64(repo.ExpiredTime), 0).UTC().Format(time.RFC3339)
		...
		item := ctyunEbsBackupReposModel{
			...
			CreatedAt:    types.StringValue(createTimeStr),
			ExpiredAt:    types.StringValue(expireTimeStr),
