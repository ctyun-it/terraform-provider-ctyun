package iam_test

import (
	"fmt"
	"github.com/ctyun-it/terraform-provider-ctyun/internal/extend/terraform"
	"os"
	"testing"
)

const dependenceDir = "testdata/dependence"

type Dependence struct {
	userID    string
	groupID   string
	groupID2  string
	authCode  string
	authCode2 string
	policyID  string
	policyID2 string
}

var dependence Dependence

func TestMain(m *testing.M) {
	if skip := os.Getenv("SKIP_IAM_TEST"); skip != "" {
		return
	}
	// 初始化依赖资源
	fmt.Println("开始初始化依赖资源")
	outputs, err := terraform.ApplyResource(dependenceDir)
	if err != nil {
		fmt.Println(err)
		terraform.DestroyResource(dependenceDir)
		os.Exit(1)
	}
	dependence = Dependence{
		userID:    outputs["user_id"].Value,
		groupID:   outputs["group_id"].Value,
		groupID2:  outputs["group_id2"].Value,
		authCode:  outputs["auth_code"].Value,
		authCode2: outputs["auth_code2"].Value,
		policyID:  outputs["policy_id"].Value,
		policyID2: outputs["policy_id2"].Value,
	}
	fmt.Println("依赖资源初始化完毕")

	// 执行测试用例
	code := m.Run()

	fmt.Println("开始清理依赖资源")
	// 清理依赖资源
	terraform.DestroyResource(dependenceDir)
	fmt.Println("依赖资源清理完毕")

	os.Exit(code)
}
