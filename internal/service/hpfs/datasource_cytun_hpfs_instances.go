...
func (c *CtyunHpfsInstances) Read(ctx context.Context, request datasource.ReadRequest, response *datasource.ReadResponse) {
	...
	for _, hpfsItem := range hpfsList {
		...
		hpfsInstance.CreateTime = types.StringValue(utils.FromUnixToUTC(hpfsItem.CreateTime))
		hpfsInstance.UpdateTime = types.StringValue(utils.FromUnixToUTC(hpfsItem.UpdateTime))
...
type CtyunHpfsInstancesModel struct {
	...
	SfsID         types.String `tfsdk:"id"`             // 并行文件唯一ID
	SfsSize       types.Int32  `tfsdk:"size"`           // 大小(GB)
	SfsType       types.String `tfsdk:"type"`           // 文件系统类型
	SfsProtocol   types.String `tfsdk:"protocol"`       // 挂载协议
	SfsStatus     types.String `tfsdk:"status"`         // 文件系统状态
	UsedSize      types.Int32  `tfsdk:"used_size"`      // 已用大小(MB)
	CreateTime    types.String `tfsdk:"create_time"`    // 创建时间
	UpdateTime    types.String `tfsdk:"update_time"`    // 更新时间
	...
	HpfsSharePath types.String `tfsdk:"share_path"`     // HPFS共享路径
...
type CtyunHpfsInstancesConfig struct {
	...
	SfsStatus     types.String              `tfsdk:"status"`     // 并行文件状态。creating/available/unusable，不传为查询全部
	SfsProtocol   types.String              `tfsdk:"protocol"`   // 挂载协议。2 种，nfs/hpfs ，不传为查询全部
	...
	HpfsInstances []CtyunHpfsInstancesModel `tfsdk:"instances"`  // hpfs列表
}
