...
- `cluster_domain` (String) 集群本地域名，创建时必填，导入时不填
- `container_runtime` (String) 容器运行时,创建时必填，导入时不填。可选containerd、docker，您可查看<a href="https://www.ctyun.cn/document/10083472/10902208">容器运行时说明</a>
- `cycle_count` (Number) 订购时长，该参数在cycle_type为month或year时才生效，当cycle_type=month，支持订购1-11个月；当cycle_type=year，支持订购1-3年
- `deploy_type` (String) 部署模式，创建时必填，导入时不填。单可用区为single，多可用区为multi
- `elb_prod_code` (String) ApiServer的ELB类型，创建时必填，导入时不填。支持standardI（标准I型），standardII（标准II型），enhancedI（增强I型），enhancedII（增强II型），higherI（高阶I型），您可查看<a href="https://www.ctyun.cn/document/10026756/10032048">ELB类型规格说明</a>
...
- `pod_subnet_id_list` (Set of String) pod子网ID列表，创建时，若网络插件选择cubecni必传，其他情况不填。需要属于所选VPC，最多支持10个子网，支持更新
...
- `az_infos` (Attributes Set) 可用区信息，包括可用区编码和该可用区下worker节点数量，支持的可用区可通过ctyun_regions查询 (see [below for nested schema](#nestedatt--slave_host--az_infos))
